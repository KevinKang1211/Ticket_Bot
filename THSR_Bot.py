    ...
    if "<@!{}> ".format(client.user.id) in message.content: #代表有人叫這個bot - bot的id = (client.user.id)
        ...
        #進入主要判斷
            ...
            if set(animalLIST).intersection(set(inputSTR)):
                ...
                await message.channel.send(response)
            if 'adultAmount' in resultDICT or 'childrenAmount' in resultDICT: #僅查詢票價
            ...
            elif bool([a for a in AroundLIST if a in inputSTR]): #僅查詢班次且有輸入「附近」之類的詞彙
